...
const NonMiningBuilderCreateSchema = z.object({
  ...
  escortFeesUSD: z.number().min(0).default(0),
  bankInterestUSD: z.number().min(0).default(0),
...
export function NonMiningBuilderCreateForm({ priceStructures }: NonMiningBuilderCreateFormProps) {
  ...
  const dduPriceUSD = acquisitionCostUSD + 
    ...
    (watchedValues.escortFeesUSD || 0) +
    (watchedValues.bankInterestUSD || 0);
    ...
    try {
      ...
      await createNonMiningBuilder({
        ...
        supplier: {
          ...
          escortFeesUSD: data.escortFeesUSD,
          bankInterestUSD: data.bankInterestUSD,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                type="number"
                step="any"
                {...form.register("escortFeesUSD", { valueAsNumber: true })}
                placeholder="0.00"
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="bankInterestUSD">Intérêts Ligne Banque (USD)</Label>
              <Input
                id="bankInterestUSD"
                type="number"
                step="any"
                {...form.register("bankInterestUSD", { valueAsNumber: true })}
                placeholder="0.00"
              />
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-muted p-4 rounded-lg">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Droit de douane: {formatCurrency(watchedValues.customsDutyUSD || 0)} + TVA Import: {formatCurrency(watchedValues.importVATUSD || 0)} + TVA nette à l’intérieur: {formatCurrency(watchedValues.internalVATUSD || 0)} + Droit de consommation: {formatCurrency(watchedValues.consumptionDutyUSD || 0)}
            </p>
          </div>
        </CardContent>
      </Card>
