...
export default function RemoveDialog({ open, setOpen, Id, nameClient }: { open: boolean; setOpen: (value: boolean) => void; Id: string; nameClient: string; }) {
  ...
  async function deleteElement() {
    const result = await deleteNonMiningBuilder({ id: Id });
    if (result?.data?.success) {
      router.push(`/dashboard/non-mining-builders`);
      router.refresh();
      setOpen(false);
    } else {
      console.error("Erreur lors de la suppression du cost build up:", result?.data?.failure);
    }
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <Label htmlFor="builderName">Titre du cost build up</Label>
          <Input
            id="builderName"
            type="text"
