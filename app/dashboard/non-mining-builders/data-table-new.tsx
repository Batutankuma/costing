...
import { columns, NonMiningBuilderWithRelations } from "./columns";
import ExportExcel from "@/components/exportExcel";
import { useRouter } from "next/navigation";
import { deleteNonMiningBuilder } from "./actions";

export function NonMiningBuildersDataTable({ data: Element }: { data: NonMiningBuilderWithRelations[] }) {
    const id = useId();
    ...
    const [data, setData] = useState<NonMiningBuilderWithRelations[]>([]);
    ...
    const handleDeleteRows = async () => {
        ...
        for (const idToDelete of idsToDelete) {
            await deleteNonMiningBuilder({ id: idToDelete });
        }
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <Input
                            ...
                            placeholder="Rechercher un cost build up..."
                            type="text"
                            aria-label="Rechercher un cost build up..."
                        />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button onClick={()=> router.push(`/dashboard/non-mining-builders/create`) }>Nouveau Cost Build Up</Button>
                    <ExportExcel
                      data={data}
                      filename="cost-build-up-non-minier"
                      mapRow={(row: NonMiningBuilderWithRelations) => {
                        return {
                          Titre: row.title,
                          Description: row.description || '',
                          'Structure': row.nonMiningPriceStructure?.nomStructure || '',
                          'Unité': row.unit === "USD_M3" ? "USD/M³" : "USD/Litre",
                          'Prix DDP (USD)': row.totals?.priceDDPUSD || '',
                          'Prix DDU (USD)': row.totals?.priceDDUUSD || '',
                          'Créé par': row.user?.name || '',
