...
export default function RemoveDialog({ open, setOpen, Id, nameClient }: { open: boolean; setOpen: (value: boolean) => void; Id: string; nameClient: string; }) {
  ...
  async function deleteElement() {
    const result = await deleteTransportRate({ id: Id });
    if (result?.data?.success) {
      router.push(`/dashboard/transport-rates`);
      router.refresh();
      setOpen(false);
    } else {
      console.error("Erreur lors de la suppression du tarif:", result?.data?.failure);
    }
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <Label htmlFor="transportRateName">Destination</Label>
          <Input
            id="transportRateName"
            type="text"
