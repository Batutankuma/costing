...
import { columns, TransportRateWithRelations } from "./columns";
import ExportExcel from "@/components/exportExcel";
import { useRouter } from "next/navigation";
import { deleteTransportRate } from "./actions";

export default function DataTables({ Element }: { Element: TransportRateWithRelations[] }) {
    const id = useId();
    ...
    const [sorting, setSorting] = useState<SortingState>([{ id: "destination", desc: false, },]);
    const [data, setData] = useState<TransportRateWithRelations[]>([]);
    ...
    const handleDeleteRows = async () => {
        ...
        for (const idToDelete of idsToDelete) {
            await deleteTransportRate({ id: idToDelete });
        }
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <Input
                            ...
                            className={cn(
                                "peer min-w-60 ps-9",
                                Boolean(table.getColumn("destination")?.getFilterValue()) && "pe-9",
                            )}
                        ...
                        {Boolean(table.getColumn("destination")?.getFilterValue()) && (
                            <button
                                className="absolute inset-y-0 end-0 flex h-full w-9 items-center justify-center rounded-e-lg text-muted-foreground/80 outline-offset-2 transition-colors hover:text-foreground focus:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/70 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
                                aria-label="Clear filter"
                                onClick={() => {
                                    table.getColumn("destination")?.setFilterValue("");
                                    if (inputRef.current) {
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button onClick={()=> router.push(`/dashboard/transport-rates/create`) }>Nouveau Tarif</Button>
                    <ExportExcel
                      data={data}
                      filename="tarifs-transport"
                      mapRow={(row: TransportRateWithRelations) => {
                        return {
                          Destination: row.destination,
                          'Tarif (USD/CBM)': row.rateUsdPerCbm,
