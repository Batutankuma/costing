...
import { ListFilter, CircleX, Columns3, ChevronUp, ChevronDown } from "lucide-react";
...
const columns: ColumnDef<BuilderRow>[] = [
  { header: "Date", accessorKey: "date", size: 150 },
  { header: "Titre", accessorKey: "title", size: 300 },
  { header: "Unité", accessorKey: "unit", size: 100 },
  {
    id: "actions",
    header: () => <span className="sr-only">Actions</span>,
    cell: ({ row }) => (
      <div className="flex gap-2 justify-end">
        <Link href={`/dashboard/build-const-kalemie/views/${row.original.id}`}>
          <Button size="sm" variant="outline">Voir</Button>
        </Link>
        <Link href={`/dashboard/build-const-kalemie/${row.original.id}`}>
