...
import { removeBuilderById } from "./actions";

export default function RemoveDialog({ open, setOpen, Id, nameClient }: { open: boolean; setOpen: (v: boolean) => void; Id: string; nameClient: string }) {
  ...
  async function deleteElement() {
    const { success } = await removeBuilderById(Id);
    if (success) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <Label htmlFor="builderName">Nom</Label>
          <Input id="builderName" type="text" placeholder={`Tapez ${nameClient} pour confirmer`} value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
        </div>
