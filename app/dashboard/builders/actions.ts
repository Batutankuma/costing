...
export const updateBuilder = actionClient
  ...
  .action(async ({ parsedInput }) => {
    ...
    revalidatePath("/dashboard/builders");
    revalidatePath(`/dashboard/builders/${id}`);
    revalidatePath(`/dashboard/builders/views/${id}`);
...
/**
 * @description Supprime un builder par son ID via actionClient.
 */
export const deleteBuilder = actionClient
  ...
  .action(async ({ parsedInput }) => {
    try {
      ...
      console.error("Erreur lors de la suppression du builder:", error);
      return { failure: "Impossible de supprimer le builder." };
    }
  });
