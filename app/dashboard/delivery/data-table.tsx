                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <Input
                            ...
                            className={cn(
                                "peer min-w-60 ps-9",
                                Boolean(table.getColumn("note")?.getFilterValue()) && "pe-9",
                            )}
                            value={(table.getColumn("note")?.getFilterValue() ?? "") as string}
                            onChange={(e) => table.getColumn("note")?.setFilterValue(e.target.value)}
                            placeholder="Filtrer par note, type d'avion ou vol..."
                            type="text"
                            aria-label="Filtrer par note, type d'avion ou vol..."
                        />
                        <div className="pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 text-muted-foreground/80 peer-disabled:opacity-50">
                            <ListFilter size={16} strokeWidth={2} aria-hidden="true" />
                        </div>
                        {Boolean(table.getColumn("note")?.getFilterValue()) && (
                            <button
                                className="absolute inset-y-0 end-0 flex h-full w-9 items-center justify-center rounded-e-lg text-muted-foreground/80 outline-offset-2 transition-colors hover:text-foreground focus:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/70 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
                                aria-label="Clear filter"
                                onClick={() => {
                                    table.getColumn("note")?.setFilterValue("");
                                    if (inputRef.current) {
