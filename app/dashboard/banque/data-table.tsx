...
import { columns, BanqueWithRelations } from "./columns";
...
export default function DataTables({ Element }: { Element: BanqueWithRelations[] }) {
    ...
    const [data, setData] = useState<BanqueWithRelations[]>([]);
    ...
    const handleDeleteRows = async () => {
        ...
        for (const idToDelete of idsToDelete) {
            await deleteBanque({ id: idToDelete });
        }
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
                ...
                <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col gap-2 max-sm:items-center sm:flex-row sm:gap-4">
                                    ...
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            {table.getSelectedRowModel().rows.length} banque{" "}
                    ...
                    <Button onClick={()=> router.push(`/dashboard/banque/create`) }>Nouvelle Banque</Button>
                    <ExportExcel
                      data={data}
                      filename="banques"
                      mapRow={(row: BanqueWithRelations) => {
                        return {
                          Nom: row.nom,
                          'Numéro de Compte': row.numeroCompte,
                          Devise: row.devise,
                          SWIFT: row.swift || '',
                          'Email Gestionnaire': row.mailGestionnaire || '',
                          'Contact Gestionnaire': row.contactGestionnaire || '',
