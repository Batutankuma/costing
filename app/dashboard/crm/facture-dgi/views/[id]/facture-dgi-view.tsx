...
import { findFactureDgiById } from "@/app/dashboard/crm/facture-dgi/actions";
...
interface FactureDgiViewClientProps {
  factureDgiId: string;
}

export default function FactureDgiViewClient({ factureDgiId }: FactureDgiViewClientProps) {
  const router = useRouter();
  const [factureDgi, setFactureDgi] = useState<FactureDgi | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      setFactureDgi(result as unknown as FactureDgi);
      setLoading(false);
    })();
  }, [factureDgiId, router]);

  if (loading || !factureDgi) {
    return (
      <div className="p-6">
        <p>Chargement de la facture DGI...</p>
      </div>
    );
  }

  const formattedDate = format(factureDgi.invoiceDate, "d MMMM yyyy", { locale: fr });
  // Par défaut 7 jours si non spécifié (non présent dans le modèle DGIFacture)
  const dueDate = format(new Date(factureDgi.invoiceDate.getTime() + 7 * 86400000), "d MMMM yyyy", { locale: fr });
    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="shadow-lg">
        <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <CardTitle className="text-2xl">Facture DGI {factureDgi.invoiceNumber}</CardTitle>
            <CardDescription>Émise le {formattedDate} • Échéance {dueDate}</CardDescription>
          </div>
          <QRCode value={factureDgi.invoiceNumber} size={96} />
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <h3 className="text-sm uppercase text-muted-foreground">Vendeur</h3>
              <p className="font-semibold">{factureDgi.vendorName}</p>
              <p className="text-sm text-muted-foreground whitespace-pre-line">{COMPANY_INFO.address}</p>
              {factureDgi.vendorNif && (
                <p className="text-sm">
                  <span className="font-medium">NIF:</span> {factureDgi.vendorNif}
                </p>
              )}
            </div>
            <div className="space-y-2">
              <h3 className="text-sm uppercase text-muted-foreground">Client</h3>
              <p className="font-semibold">{factureDgi.clientName}</p>
              {factureDgi.clientAddress && <p className="text-sm text-muted-foreground whitespace-pre-line">{factureDgi.clientAddress}</p>}
              {factureDgi.clientNif && (
                <p className="text-sm">
                  <span className="font-medium">NIF Client:</span> {factureDgi.clientNif}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="text-sm flex gap-4 flex-wrap">
              <Badge variant="outline">Devise: {factureDgi.currency}</Badge>
              <Badge variant="outline">Échéance: 7 jours</Badge>
            </div>
          </div>
                ...
                {factureDgi.lines.map((line, idx) => (
                  <tr key={idx} className="border-t">
                    ...
                    <td className="px-4 py-2 text-right">
                      {line.unitPrice.toFixed(4)} {factureDgi.currency}
                    </td>
                    <td className="px-4 py-2 text-right">
                      {(line.quantity * line.unitPrice).toFixed(2)} {factureDgi.currency}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              {factureDgi.notes && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm whitespace-pre-line">{factureDgi.notes}</p>
                </div>
              )}
            </div>
            <div className="space-y-2 bg-muted/40 rounded-lg p-4">
              <div className="flex justify-between">
                ...
                <span>
                  {factureDgi.totalHT.toFixed(2)} {factureDgi.currency}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Total TVA (16%)</span>
                <span>
                  {factureDgi.totalTVA.toFixed(2)} {factureDgi.currency}
                </span>
              </div>
              ...
              <div className="flex justify-between font-semibold text-lg">
                <span>Total TTC</span>
                <span>
                  {factureDgi.totalTTC.toFixed(2)} {factureDgi.currency}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="text-sm text-muted-foreground">
            Montant en lettres: <span className="italic">{amountToWordsFR(factureDgi.totalTTC, factureDgi.currency)}.</span>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="facture-print" style={{ fontFamily: "Arial, sans-serif", fontSize: "11px", lineHeight: "1.4", color: "#000", width: "100%", padding: "0", boxSizing: "border-box" }}>
        ...
        <div style={{ padding: "0 20mm 15mm 20mm" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <div style={{ fontSize: "16px", fontWeight: "bold" }}>N° FACTURE DGI {factureDgi.invoiceNumber}</div>
            <div style={{ fontSize: "11px" }}>Kinshasa {format(factureDgi.invoiceDate, "dd MMMM yyyy", { locale: fr })}</div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginBottom: "20px" }}>
            <div>
              ...
              <div style={{ fontSize: "10px", lineHeight: "1.6" }}>
                <div><span style={{ fontWeight: "bold" }}>Nom de la société</span> : {factureDgi.clientName}</div>
                {factureDgi.clientAddress && <div><span style={{ fontWeight: "bold" }}>Adresse</span> : {factureDgi.clientAddress}</div>}
                {factureDgi.clientNif && <div><span style={{ fontWeight: "bold" }}>NIF Client</span> : {factureDgi.clientNif}</div>}
              </div>
            </div>
            <div style={{ fontSize: "10px" }}>
              <div style={{ marginBottom: "4px" }}><span style={{ fontWeight: "bold" }}>Échéance:</span> 7 jours</div>
            </div>
          </div>
          ...
          <table style={{ width: "100%", fontSize: "10px", marginBottom: "15px", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: "#f5f5f5" }}>
                ...
                <th style={{ padding: "8px", textAlign: "right", border: "1px solid #333", fontWeight: "bold" }}>PRIX TOTAL ({factureDgi.currency})</th>
              </tr>
            </thead>
            <tbody>
              {factureDgi.lines.map((line, index) => {
          ...
          <div style={{ marginTop: "20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "30px" }}>
            ...
            <div style={{ flex: "1" }}>
              ...
              <div style={{ fontSize: "11px" }}>
                <span style={{ fontWeight: "bold" }}>Montant en lettre:</span> {amountToWordsFR(factureDgi.totalTTC, factureDgi.currency)}.
              </div>
            </div>
            ...
            <div style={{ border: "2px solid #333", padding: "12px", width: "240px", fontSize: "10px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                ...
                <span style={{ fontWeight: "bold" }}>{factureDgi.totalHT.toLocaleString("fr-FR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                <span>TVA</span>
                <span>{factureDgi.totalTVA.toLocaleString("fr-FR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #333", paddingTop: "8px", marginTop: "8px", fontSize: "14px", fontWeight: "bold" }}>
                <span>Total TTC</span>
                <span>{factureDgi.totalTTC.toLocaleString("fr-FR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
              </div>
            </div>
          </div>
            ...
            <div style={{ flex: "1", paddingRight: "40px", display: "flex", alignItems: "flex-start", gap: "20px" }}>
              <div style={{ marginTop: "15px" }}>
                <div style={{ display: "inline-block", padding: "8px", backgroundColor: "white", border: "1px solid #ccc" }}>
                  <QRCode value={factureDgi.invoiceNumber} size={80} style={{ height: "auto", maxWidth: "100%", width: "100%" }} />
                </div>
              </div>
