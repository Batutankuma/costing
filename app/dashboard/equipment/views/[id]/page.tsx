...
export default async function ViewEquipmentPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="p-6 space-y-4 max-w-xl mx-auto">
        <div className="text-destructive bg-destructive/10 p-4 rounded-md">
          <p className="font-medium">Equipment introuvable.</p>
        </div>
        <Button variant="outline" asChild>
          <Link href="/dashboard/equipment">Retour à la liste</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
                ...
                Détails du Equipment: {Equipment.name}
              </h1>
              <p className="text-muted-foreground">Visualisation complète du Equipment</p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Détails du Equipment</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-lg">{Equipment.capacity} {Equipment.unit}</p>
            </div>
            <div>
              <Label className="text-sm font-medium text-muted-foreground">Niveau actuel</Label>
              <p className="text-lg font-semibold text-primary">{Equipment.currentLevel} {Equipment.unit}</p>
            </div>
            <div>
              <Label className="text-sm font-medium text-muted-foreground">Dépôt</Label>
              <p className="text-lg">{Equipment.depot?.name ?? "N/A"}</p>
            </div>
            <div>
              <Label className="text-sm font-medium text-muted-foreground">Produit</Label>
              <p className="text-lg">{Equipment.produit?.name ?? "N/A"}</p>
            </div>
            <div>
              <Label className="text-sm font-medium text-muted-foreground">Taux de remplissage</Label>
              <div className="mt-2">
                <Badge variant={
                  (Equipment.currentLevel / Equipment.capacity) > 0.8 ? "default" : 
                  (Equipment.currentLevel / Equipment.capacity) > 0.5 ? "secondary" : 
                  "destructive"
                }>
                  {((Equipment.currentLevel / Equipment.capacity) * 100).toFixed(1)}%
                </Badge>
              </div>
            </div>
          </div>
