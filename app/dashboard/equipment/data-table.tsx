...
export default function DataTables({ Element }: { Element: Equipment[] }) {
    const id = useId();
    ...
    const handleDeleteRows = async () => {
        ...
        for (const idToDelete of idsToDelete) {
            await removeByIdAction(idToDelete);
        }
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button onClick={()=> router.push(`/dashboard/equipment/create`) }>Nouvel Équipement</Button>
                    <ExportExcel
                      data={data}
                      filename="equipments"
                      mapRow={(row: Equipment) => {
                        return {
                          Nom: row.name,
                          Dépôt: row.depot?.name || '',
                          Produit: row.produit?.name || '',
                          Capacité: row.capacity,
                          'Niveau Actuel': row.currentLevel,
                          Unité: row.unit,
                          'Créé le': row.createdAt?.toLocaleDateString('fr-FR') || 'N/A',
                        };
                      }}
                    />
                </div>
