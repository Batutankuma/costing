...
import { columns, ClientWithRelations } from "./columns";
...
export default function DataTables({ Element }: { Element: ClientWithRelations[] }) {
    ...
    const [data, setData] = useState<ClientWithRelations[]>([]);
    useEffect(() => { setData(Element); }, [Element]);

    const filteredData = useMemo(() => {
        return data.filter((row) => {
            const okStatus = selectedStatus === "ALL" || row.status === selectedStatus;
            return okStatus;
        });
    }, [data, selectedStatus]);
    ...
    const handleDeleteRows = async () => {
        ...
        for (const idToDelete of idsToDelete) {
            await deleteClient({ id: idToDelete });
        }
            ...
            <DropdownMenuContent align="end">
                ...
                <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col gap-2 max-sm:items-center sm:flex-row sm:gap-4">
                                    ...
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            {table.getSelectedRowModel().rows.length} client{" "}
                    ...
                    <ExportExcel
                      data={filteredData}
                      filename="clients"
                      mapRow={(row: ClientWithRelations) => {
                        return {
                          Nom: row.name,
                          Société: row.company || '',
                          Email: row.email || '',
