...
import { ArrowLeft, Package, Save, Calendar, Hash, Warehouse, Users, Calculator, DollarSign, AlertTriangle, ArrowRightLeft } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function StockCreateForm({
  ...
  const currentDepot = watch("depotId");
  const currentProduit = watch("produitId");
  const currentFournisseur = watch("fournisseurId");
  const currentClient = watch("clientId");

  const commandesEligibles = commandeSuggestions.filter((cmd) =>
    ["CONFIRMED", "PARTIALLY_RECEIVED"].includes(cmd.status)
  );
  const selectedCommande = commandesEligibles.find((cmd) => cmd.id === selectedCommandeId);

  useEffect(() => {
    if (stockType !== "ENTREE" && selectedCommandeId) {
      setSelectedCommandeId("");
      setValue("reference", "", { shouldDirty: true, shouldValidate: true });
      setValue("fournisseurId", undefined, { shouldDirty: true });
    }
  }, [stockType, selectedCommandeId, setValue]);

  const handleCommandeSelect = (value: string) => {
    setSelectedCommandeId(value);
    const commande = commandesEligibles.find((cmd) => cmd.id === value);
    if (!commande) {
      setValue("reference", "", { shouldDirty: true, shouldValidate: true });
      return;
    }
    setValue("reference", commande.reference ?? "", { shouldDirty: true, shouldValidate: true });
    if (commande.fournisseurId) {
      setValue("fournisseurId", commande.fournisseurId, { shouldDirty: true });
    }
    if (commande.depotId) {
      setValue("depotId", commande.depotId, { shouldDirty: true });
    }
    if (commande.produitId) {
      setValue("produitId", commande.produitId, { shouldDirty: true });
    }
  };

  const clearCommandeSelection = () => {
    setSelectedCommandeId("");
    setValue("reference", "", { shouldDirty: true, shouldValidate: true });
    setValue("fournisseurId", undefined, { shouldDirty: true });
  };
  ...
  const onSubmit = async (data: StockFormData) => {
    ...
    try {
      ...
      const result = await createAction(stockData);

      if (!result?.data?.success || result?.data?.failure) {
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result?.data?.failure || "Une erreur est survenue lors de l'enregistrement."
        });
        return;
      }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Remplissez les informations ci-dessous pour créer un nouveau mouvement de stock
        </CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    {...register("reference")}
                    readOnly={stockType === "ENTREE" && Boolean(selectedCommandeId)}
                    className="h-10 pl-10"
                  />
                </div>
                {errors.reference && (
                ...
                {stockType === "ENTREE" && selectedCommande && (
                  <p className="text-xs text-muted-foreground">
                    Référence imposée par le bon de commande {selectedCommande.reference}.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            {stockType === "ENTREE" && (
              <div className="space-y-2">
                ...
                <Select
                  value={selectedCommandeId || undefined}
                  onValueChange={(value: string) => handleCommandeSelect(value)}
                >
                  <SelectTrigger id="commandeId" className="h-10">
                    <SelectValue placeholder="Associer un bon de commande" />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    {commandesEligibles.map((commande) => (
                      <SelectItem key={commande.id} value={commande.id}>
                        {commande.reference} • {commande.fournisseurNom ?? "Fournisseur ?"}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {selectedCommande && (
                  <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    <span>
                      Fournisseur : <strong>{selectedCommande.fournisseurNom ?? "N/A"}</strong>
                    </span>
                    <span>
                      Produit : <strong>{productSuggestions.find((p) => p.id === selectedCommande.produitId)?.name ?? selectedCommande.produitId ?? "N/A"}</strong>
                    </span>
                    <Button type="button" variant="ghost" size="sm" onClick={clearCommandeSelection}>
              ...
              {stockType === "ENTREE" ? (
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Select
                      value={currentFournisseur || undefined}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Select
                    value={currentProduit || undefined}
            ...
            <div className="grid grid-cols-1 gap-4">
              {stockType === "ENTREE" ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Le prix sera calculé automatiquement selon le Coût Moyen Pondéré du stock actuel
                  </p>
                </div>
              )}
            </div>
