...
export const updatePriceReference = actionClient
  ...
  .action(async ({ parsedInput }) => {
    try {
      ...
      revalidatePath("/dashboard/prices");
      revalidatePath(`/dashboard/prices/${parsedInput.id}`);
      revalidatePath(`/dashboard/prices/views/${parsedInput.id}`);
...
/**
 * @description Supprime un prix de référence par son ID via actionClient.
 */
export const deletePriceReference = actionClient
  ...
  .action(async ({ parsedInput }) => {
    try {
      ...
      return { failure: "Impossible de supprimer le prix de référence." };
    }
  });
